...
docker run -h archganglia -d -ti -v /path/to/Docklia/clone/dir/confDir/:/usr/local/etc/ -p 80:80 -p 6343:6343/udp -p 8649:8649/udp -p 8649:8649/tcp ganglia -z Africa/Johannesburg -s
# OR
docker run -d -ti -v /path/to/Docklia/clone/dir/confDir/:/usr/local/etc/ --net=host ganglia -z Africa/Johannesburg -s
```
...
- -z            Allows you to set the timezone of the container (useful for Ganglia issues sometimes)
- -s            Runs the container instance as a Ganglia server.

You can also use the -c flag instead of server to deploy a gmond client on a node.
