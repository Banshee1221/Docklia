...
while getopts "scz:" opt; do
  case $opt in
    ...
    c)
      ...
      ;;
    z)
      echo "Timezone setting to $OPTARG"
      ZONE=$OPTARG
      ;;
    \?)
...
if [ -z "$SUCCESS" ]
then
  echo "Enter a valid timezone for -c"
  exit
fi

if [ "$CLIENT" == "" ] && [ "$SERVER" == "" ]
then
  echo "Specify either -c or -s."
  exit
fi

if [ "$CLIENT" == "$SERVER" ]
then
  echo "Can't specify both client and server!"
  exit
fi

if [ "$CLIENT" == "y" ] && [ -z "$SERVER"]
then
